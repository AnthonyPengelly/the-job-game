...
# Returns the value after the last matching marker, or "" if absent.
# ---------------------------------------------------------------------------
...
has_status_lgtm()   { grep -Eq '^PIPELINE_STATUS:[[:space:]]*LGTM$'   "$1" 2>/dev/null; }
has_status_issues() { grep -Eq '^PIPELINE_STATUS:[[:space:]]*ISSUES$' "$1" 2>/dev/null; }
has_blocked()       { grep -Eq '^PIPELINE_BLOCKED:'                   "$1" 2>/dev/null; }
